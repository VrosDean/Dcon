...
void read_disk(int LBA, unsigned short* target) {
    ...
    outb(0x1F2, 1); outb(0x1F3, (unsigned char)LBA);
    outb(0x1F4, (unsigned char)(LBA >> 8)); outb(0x1F5, (unsigned char)(LBA >> 16));
    outb(0x1F7, 0x20);
    while (inb(0x1F7) & 0x80); while (!(inb(0x1F7) & 0x08));
    for (int i = 0; i < 256; i++) target[i] = inb(0x1F0) | (inb(0x1F0) << 8);
}
...
void write_disk(int LBA, unsigned short* source) {
    ...
    outb(0x1F7, 0x30);
    while (inb(0x1F7) & 0x80); while (!(inb(0x1F7) & 0x08));
    for (int i = 0; i < 256; i++) { outb(0x1F0, (unsigned char)(source[i] & 0xFF)); outb(0x1F0, (unsigned char)((source[i] >> 8) & 0xFF)); }
    outb(0x1F7, 0xE7);
}
...
void draw_ui() {
    ...
    for (int row = 0; row < 24; row++) {
        ...
        for (int col = 0; col < 80; col++) {
            int index = (row * 80 + col) * 2;
            vidptr[index] = ' '; vidptr[index + 1] = bg;
        }
    }
    for (int row = 14; row < 24; row++) {
        for (int col = 4; col < 76; col++) {
            int index = (row * 80 + col) * 2;
            vidptr[index] = ' '; vidptr[index + 1] = 0x00; 
        }
    }
...
void draw_clock() {
    ...
    h = (h & 0x0F) + ((h / 16) * 10); m = (m & 0x0F) + ((m / 16) * 10); y = (y & 0x0F) + ((y / 16) * 10);
    int pos = (0 * 80 + 65) * 2;
    vidptr[pos] = (h / 10) + '0'; vidptr[pos+2] = (h % 10) + '0'; vidptr[pos+4] = ':';
    vidptr[pos+6] = (m / 10) + '0'; vidptr[pos+8] = (m % 10) + '0'; vidptr[pos+10] = ' ';
    vidptr[pos+12] = '2'; vidptr[pos+14] = '0'; vidptr[pos+16] = (y / 10) + '0'; vidptr[pos+18] = (y % 10) + '0';
    for(int i=1; i<20; i+=2) vidptr[pos + i] = (vidptr[pos+i] & 0xF0) | 0x0F;
}

/* 5. THE COMMAND BRAIN */
void run_command() {
    if (cmd_idx == 4 && cmd_buffer[0] == 'P' && cmd_buffer[1] == 'A' && cmd_buffer[2] == 'C' && cmd_buffer[3] == 'K') {
        print_at("PackON: FETCHING 'DEAN_APP'...", 15, 5);
        unsigned short p[256]; read_disk(100, p); write_disk(200, p);
        print_at("PackON: SUCCESS! INSTALLED TO SECTOR 200", 16, 5);
    } 
    else if (cmd_idx == 2 && cmd_buffer[0] == 'W' && cmd_buffer[1] == 'I') {
        ...
        draw_ui(); cursor = 14 * 80 + 5;
    }
    for(int i=0; i<80; i++) cmd_buffer[i] = 0; cmd_idx = 0;
}
